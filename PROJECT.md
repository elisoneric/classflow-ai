...
| ADR-4 (superseded, see below) | ~~AI provider is Anthropic Claude~~, called through a `MessageInterpreter` port. Model confirmed as Claude Haiku 4.5 (`ANTHROPIC_MODEL`). | Structured output via forced tool-calling (not free-text parsing) for reliability. Provider-agnostic interface means swapping models/providers later touches one adapter, not business logic. |
| ADR-4-rev2 | **Active AI provider switched to Gemini** (`gemini-3.5-flash`, `GEMINI_API_KEY`/`GEMINI_MODEL`), still through the same `MessageInterpreter` port. | Course Rep has free Gemini API access via an existing Google subscription; Anthropic's is pay-as-you-go. `GeminiInterpreter` (`app/infrastructure/ai/gemini_interpreter.py`) forces JSON-shaped output via `response_schema` structured output rather than tool-calling. `AnthropicInterpreter` is kept in the codebase as a working alternate implementation of the same port (ADR-6) — switching back is a one-line change in `app/infrastructure/jobs/tasks.py`, not a rewrite. This is the port abstraction earning its cost: the swap touched one adapter file, one import, and zero business logic. |
...
**Output:** forced structured JSON via the active provider's native mechanism (never free-text parsing). The Gemini adapter (active, ADR-4) uses `response_schema` structured output — Gemini's `Schema` type is an OpenAPI-3.0 subset, so optional fields use `nullable: true` rather than a JSON-Schema type union. The Anthropic adapter (kept as an alternate implementation behind the same `MessageInterpreter` port, ADR-6) instead forces a single `record_interpretation` tool call via `tool_choice`. Both produce the same shape:
